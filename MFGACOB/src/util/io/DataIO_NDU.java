...
public class DataIO_NDU {
    ...
    public static Task TaskInit(File file) throws FileNotFoundException {
        ...
        while (Main.scanner.hasNextLine()) {
            ...
            String[] spline = line.trim().split("\\s+");
            //System.out.println(line);
            if (spline.length == 1 && spline[0].equals(Integer.toString(targetNode))) {
                nodeList.get(nodeList.size()-1).domain = domainIdx;
                break;
            }
            else {
                for(int i = 0; i < spline.length; i++) {
                    int nodeIdx;
                    try {
                        nodeIdx = Integer.parseInt(spline[i]) - 1;
                        nodeList.get(nodeIdx).domain = domainIdx;
                    } catch (NumberFormatException ex) {
                        System.out.println("ERROR 404: Number format exception!");
                    }
                }
                domainIdx++;
            }
        ...
        int inNode = -1, outNode = -1, weight = 0;
        while (Main.scanner.hasNextLine()) {
            String line = Main.scanner.nextLine();
            String[] spline = line.trim().split("\\s+");
            inNode = Integer.parseInt(spline[0]);
			outNode = Integer.parseInt(spline[1]);
			weight = Integer.parseInt(spline[2]);

            Edge e = new Edge(inNode, outNode, weight, Settings.INIT_PHEROMONE, 0, false);
        ...
        Main.scanner.close();
        Task task = new Task(nodeNumber, domainNumber, edgeNumber, sourceNode, targetNode, false, nodeList);
			...
			for (int k = 0; k < task.nodeList.get(j).outNodeList.size(); k++) {
				...
				for (int l = 0; l < task.nodeList.get(j).outNodeList.get(k).size(); l++) {
					System.out.println("          	     Edge " + (l + 1) + ": "
                            ...
                            + task.nodeList.get(j).domain + "; "
							+ task.nodeList.get(task.nodeList.get(j).outNodeList.get(k).get(l).outNode-1).domain + "; "
